...
dragStage1MC.on("pan", handleDragGmply);

// var global
var maxYTopGmply = 0;
var maxYBottomGmply = -900;
var maxXLeftGmply = 0;
var maxXRightGmply = -1490;
var lastPosXGmply = 0;
var lastPosYGmply = 0;
var isDraggingGmply = false;
function handleDragGmply(ev) {
    var elem = dragStage1;
    
    // DRAG STARTED
    if ( ! isDraggingGmply ) {
        isDraggingGmply = true;
        lastPosXGmply = elem.offsetLeft;
        lastPosYGmply = elem.offsetTop;
    }

    // move Y position
    var posYGmply = 0;
    var keepYTopGmply = (ev.deltaY + lastPosYGmply) > maxYTopGmply;
    var keepYBottomGmply = (ev.deltaY + lastPosYGmply) < maxYBottomGmply;
    if(keepYTopGmply){
        posYGmply = maxYTopGmply;
    }else if(keepYBottomGmply){
        posYGmply = maxYBottomGmply;
    }else{
        posYGmply = ev.deltaY + lastPosYGmply;
    }
    elem.style.top = posYGmply + "px";

    // move X position
    var posXGmply = 0;
    var keepXLeftGmply = (ev.deltaX + lastPosXGmply) > maxXLeftGmply;
    var keepXRightGmply = (ev.deltaX + lastPosXGmply) < maxXRightGmply;
    if(keepXLeftGmply){
        posXGmply = maxXLeftGmply;
    }else if(keepXRightGmply){
        posXGmply = maxXRightGmply;
    }else{
        posXGmply = ev.deltaX + lastPosXGmply;
    }
    elem.style.left = posXGmply + "px";
    
    // DRAG ENDED
    if (ev.isFinal) {
        isDraggingGmply = false;
    }
}
